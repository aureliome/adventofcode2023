/*
  ...
  14 instances of card 5, and 1 instance of card 6. In total, this example pile of 
...
const main = (input) => {
  // split the lines
  let cards = splitLines(input);

  cards = cards.map((card, index) => {
    let [, numbers] = card.split(":");
    const [winningNumbersString, yourNumbersString] = numbers.split(" | ");
    ...
    const wins = winningNumbers.reduce((acc, winningNumber) => {
  ...
  while (!finished) {
    ...
    finalCards.forEach((card, index) => {
      card.forEach((cardToDuplicate) => {
        finished = false;
        finalCards.push(cards[cardToDuplicate]);
      });
      finalCards[index] = [];
    });
  }
