...
const main = (input, expansionFactor) => {
  // get the matrix
  ...
  const map = matrix.reduce((acc, row, rowIndex) => {
    row.forEach((element, columnIndex) => {
      if (element === "#") {
        const columnsToAdd =
          emptyColumns.filter((emptyColumn) => emptyColumn < columnIndex)
            .length *
          (expansionFactor - 1);
        const rowsToAdd =
          emptyRows.filter((emptyRow) => emptyRow < rowIndex).length *
          (expansionFactor - 1);
        acc.push([columnIndex + columnsToAdd, rowIndex + rowsToAdd]);
      }
    });
    return acc;
  }, []);
...
console.log(`The sum is ${main(realInput, 1000000)}`);
